...
import { HttpRouter, HttpServer, HttpServerResponse, HttpMiddleware, HttpApiError, HttpServerRequest } from "@effect/platform"
import { Layer, Effect, Ref, Context } from "effect"
import { HttpApiBuilder } from "@effect/platform"
import { HttpApi, Todo, User, CurrentUser, CreateTodoRequest, UpdateTodoRequest } from "../shared/httpapi"
...
// Todos handlers implementation
const todosHandlers = HttpApiBuilder.group(HttpApi, "todos", (handlers) =>
  Effect.gen(function* () {
...
// Users handlers implementation
const usersHandlers = HttpApiBuilder.group(HttpApi, "users", (handlers) =>
  handlers
...
// Create the HTTP app
const httpApp = HttpApiBuilder.serve(HttpApi).pipe(
  Layer.provide(todosHandlers),
...
const mainRouter = HttpRouter.empty.pipe(
  HttpRouter.mount("/api", httpApp),
  HttpRouter.mount("/", healthRouter)
);

// Create server with middleware
const ServerLive = HttpServer.serve(HttpMiddleware.logger).pipe(
  Layer.provide(HttpServer.layer(() => mainRouter, { port: 3001 })),

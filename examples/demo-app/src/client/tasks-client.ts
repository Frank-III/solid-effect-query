...
export class TasksClient extends Effect.Service<TasksClient>()('TasksClient', {
  ...
  dependencies: [
    RpcClient.layerProtocolHttp({
      url: '/rpc'  // Use proxy path instead of direct URL
    }).pipe(
...
const runtime = ManagedRuntime.make(TasksClient.Default).pipe(
  ...
  Effect.tapError((error) => Effect.logError("TasksClient runtime error:", error)),
  Effect.runSync
)
    ...
    const result = yield* service.client("getTasks", {}).pipe(
      ...
      Effect.tap((either) => 
        Either.isRight(either) 
          ? Effect.log("[TasksClient] Test call succeeded", { tasksCount: either.right.length })

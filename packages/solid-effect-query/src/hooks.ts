  ...
  >(
    ...
    const baseResults = createQuery(() => {
      ...
      if (typeof queryFn === "function") {
        processedOptions.queryFn = async (args: QueryFunctionContext<TQueryKey, never>) => {
            ...
            return error
              ? Either.match(
                  Cause.failureOrCause(
                    error as unknown as Cause.Cause<TError>,
                  ),
                  {
  ...
  > {
    ...
    const baseResults = createMutation(() => {
      ...
      if (typeof onSuccess === "function") {
        processedOptions.onSuccess = async (...args: [TData, TVariables, TContext]) => {
          return await runPromise(
            onSuccess(...args).pipe(
      ...
      if (typeof onError === "function") {
        processedOptions.onError = async (baseError: any, ...args: [TVariables, TContext | undefined]) => {
          const error = throwOnDefect
            ? Either.match(Cause.failureOrCause(baseError as Cause.Cause<TError>), {
                onLeft: (error) => error as unknown as TExposedError,
                onRight: (_cause) => {
                  throw new Error(
                    "non fail cause with throwOnDefect: true should have thrown already",
      ...
      if (typeof onSettled === "function") {
        processedOptions.onSettled = async (data: TData | undefined, baseError: any, ...args: [TVariables, TContext | undefined]) => {
